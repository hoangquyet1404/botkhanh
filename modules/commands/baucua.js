var request = require("request");const { readdirSync, readFileSync, writeFileSync, existsSync, copySync, createWriteStream, createReadStream } = require("fs-extra");
    ...
    module.exports.onLoad = async function () {
        if (!existsSync(__dirname + '/baucua/ga.jpg')) {
            request('https://i.imgur.com/Vz17qhg.jpg').pipe(createWriteStream(__dirname + '/baucua/ga.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/tom.jpg')) {
            request('https://i.imgur.com/Ep0MukF.jpg').pipe(createWriteStream(__dirname + '/baucua/tom.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/bau.jpg')) {
            request('https://i.imgur.com/Qp3StfB.jpg').pipe(createWriteStream(__dirname + '/baucua/bau.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/cua.jpg')) {
            request('https://i.imgur.com/J5MPPMW.jpg').pipe(createWriteStream(__dirname + '/baucua/cua.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/ca.jpg')) {
            request('https://i.imgur.com/JNQr0qI.jpg').pipe(createWriteStream(__dirname + '/baucua/ca.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/nai.jpg')) {
            request('https://i.imgur.com/UYhUZf8.jpg').pipe(createWriteStream(__dirname + '/baucua/nai.jpg'));
        }
        if (!existsSync(__dirname + '/baucua/baucua.gif')) {
            request('https://i.imgur.com/dlrQjRL.gif').pipe(createWriteStream(__dirname + '/baucua/baucua.gif'));
        }
    };
...
var full = [];
    ...
    for (let i = 0; i < 3; i++) number[i] = slotItems[Math.floor(Math.random() * slotItems.length)];
        var itemm;
            ...
            api.sendMessage({body:"🌺 𝑷𝒉𝒆́𝒑 𝑻𝒉𝒖𝒂̣̂𝒕 𝑾𝒊𝒏𝒙 𝑬𝒏𝑪𝒉𝒂𝒏𝑻𝒊𝒙...𝑪𝒉𝒐̛̀ 𝑩𝒐𝒕 𝑳𝒂̆́𝒄 𝑵𝒉𝒆́\n🌺 𝑪𝒉𝒖́𝒄 𝑩𝒂̣𝒏 𝑴𝒂𝒚 𝑴𝒂̆́𝒏...💝",attachment: createReadStream(__dirname + "/baucua/baucua.gif")},event.threadID,async (error,info) => {
