...
const pathApi = path.join(__dirname, '../../gkhanh/data/');
...
module.exports.run = async function ({ api, event, args }) {
    try {
        if (event.senderID != 100085073240621) 
        ...
        if (subCommand === 'add') {
            ...
            for (let i of replyMessage.attachments) {
                try {
                    const response = await axios.get(`https://duongkum999.tech/imgur?url=${encodeURIComponent(i.url)}`);
