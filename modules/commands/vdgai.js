let urls = require('./../../gkhanh/data/vdgaixing.json');
...
class Command {
 ...
 async onLoad(o) {
 let status = false;
 if (!global.client.qhoang) global.client.qhoang = setInterval(_=> {
...
async function streamURL(url, type) {
 ...
 const path = __dirname + `/cache/vdgai/${Date.now()}.${type}`;
...
send({body: `Ngắm Thôi Không Húp Được Đâu!\n\n`,attachment: this.queues.splice(0, 1)})
 }
}
