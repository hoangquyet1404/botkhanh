...
module.exports.run = async function ({ Currencies, api, event, args, Users,permssion }) {
 ...
 return api.sendMessage(`Đã chuyển cho ${name} ${bet}$`,event.threadID)}
  ...
  if (money === Infinity) return api.sendMessage(`con lồn ${name} có vô hạn tiền`,event.threadID)
  if (money === null) return api.sendMessage(`📝con lồn ${name} có 0$ nghèo thì cút đi`,event.threadID)
  if (!args[0] || !args[1]) return api.sendMessage(`📝con lồn ${name} có ${money}$`,event.threadID)
}