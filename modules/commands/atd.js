...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_youtube.test(el)) {
            const data = (await axios.get(`https://api-dowww.onrender.com/youtube?url= ${el}`)).data.result,
        ...
        if (regEx_instagram.test(el)) {
            const data = (await axios.get(` //https://93a0-43-134-186-138.ngrok-free.app/instagram/downloadpost?url=//  ` + el)).data;
