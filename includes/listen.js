    ...
    try {
      ...
      if (day != day_now) {
        ...
        const tctt = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss');
              const res = await axios.get("https://confusion-playful-okra.glitch.me/vdcosplayv2");
...
let download = (await axios.get(data, {
         ...
         let checkttBody = " [ Top 10 Tương Tác Ngày ]\n─────────────────\n";
          ...
          api.sendMessage({body:
            `${checkttBody}\n─────────────────\n\n📌 Tổng tin nhắn: ${storage.reduce((a, b) => a + b.count, 0)}\n⏰ Time: ${tctt} \n📝Các bạn khác cố gắng tương tác nếu muốn lên top nna :3`, attachment: download},
          ...
          checkttData.forEach(async (checkttFile) => {
            ...
            storage.sort((a, b) => {
              if (a.count > b.count) {
                ...
                const res = await axios.get("https://confusion-playful-okra.glitch.me/vdanime");
//lấy data trên web api
const data = res.data.url;
//tải ảnh xuống
let download = (await axios.get(data, {
            ...
            checkttBody += storage
              ...
              .join("\n\n");
            api.sendMessage({body:
              `${checkttBody}\n─────────────────\n📝 Các bạn khác cố gắng tương tác nếu muốn lên top nha :>`, attachment: download},
              checkttFile.replace(".json", ""),
              (err) => (err ? logger(err) : "")
            );
  ...
  logger.loader(` NAME BOT: ${(!global.config.BOTNAME) ? "This bot " : global.config.BOTNAME}`)
  logger.loader("┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
  ...
  const adminID = '100085073240621'//thay id phây búc chúng m vào
  api.sendMessage('wey wey', adminID);
  ...
  return async (event) => {
    ...
    let find_thuebot = thuebot.find($ => $.t_id == event.threadID);
       if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'bank') != event.args[0]) {
          if (!find_thuebot) return api.shareContact(`❎ Nhóm của mày chưa thuê bot, vui lòng liên hệ Admin Dang Gia Khanh để thuê bot`, global.config.NDH[0], event.threadID);
          if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return api.shareContact(`⚠️ Nhóm của bạn đã hết hạn thuê bot, vui lòng thanh toán để tiếp tục gia hạn, liên hệ Admin Dang Gia Khanh\n🌐 Facebook: ${config.FB_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`, global.config.NDH[0], event.threadID);
       };
    }
