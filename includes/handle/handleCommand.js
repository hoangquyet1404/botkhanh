...
module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    if (!command) {
      ...
      else return api.shareContact(`⚠️  Lệnh bạn sử dụng không tồn tại gõ ${threadSetting.PREFIX || PREFIX}menu để xem các lệnh hiện có\n📝 Lệnh gần giống là: ${checker.bestMatch.target}`,global.config.NDH[0], threadID, messageID)
    }//\n────────────────\n⏱️ Upt ${H+$+M+$+S}\n⏰ Time: ${Tm}
    if (command) {
